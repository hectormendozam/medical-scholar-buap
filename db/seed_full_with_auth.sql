-- Corrected full seed: create auth.users first so the profiles trigger creates matching profiles
-- Run this in Supabase SQL editor as project owner (admin). This will avoid FK violations on public.profiles.

BEGIN;

-- 1) Create specific admin & instructor users with deterministic UUIDs (match values used elsewhere)
INSERT INTO auth.users (id, aud, role, email, email_confirmed_at, raw_user_meta_data, created_at)
VALUES
  ('11111111-1111-1111-1111-111111111111', 'authenticated', NULL, 'admin.demo@example.com', now(), jsonb_build_object('full_name','Admin Demo'), now()),
  ('22222222-2222-2222-2222-222222222222', 'authenticated', NULL, 'instructor.demo@example.com', now(), jsonb_build_object('full_name','Instructor Demo'), now())
ON CONFLICT (id) DO NOTHING;

-- 2) Create 20 student auth users (emails predictable)
INSERT INTO auth.users (id, aud, role, email, email_confirmed_at, raw_user_meta_data, created_at)
SELECT uuid_generate_v4(), 'authenticated', NULL, format('student%d+seed@example.com', g), now(), jsonb_build_object('full_name', 'Alumno Seed ' || g), now()
FROM generate_series(1,20) g
ON CONFLICT (email) DO NOTHING;

-- At this point the trigger public.handle_new_user should have created matching rows in public.profiles.
-- Verify profiles count for these emails if needed.
...
-- 4) Assign first 10 students to Cardiología and next 10 to Urgencias
WITH students AS (
  SELECT id FROM public.profiles WHERE email LIKE 'student%+seed@example.com' ORDER BY created_at
), c1 AS (
  SELECT id FROM public.courses WHERE name = 'Cardiología 101'
), c2 AS (
  SELECT id FROM public.courses WHERE name = 'Urgencias - Nivel Básico'
)
INSERT INTO public.course_members (course_id, user_id)
SELECT (SELECT id FROM c1), s.id
FROM students s
WHERE (SELECT row_number FROM (SELECT id, row_number() OVER () as row_number FROM students) t WHERE t.id = s.id) <= 10
ON CONFLICT DO NOTHING;

INSERT INTO public.course_members (course_id, user_id)
SELECT (SELECT id FROM c2), s.id
FROM students s
WHERE (SELECT row_number FROM (SELECT id, row_number() OVER () as row_number FROM students) t WHERE t.id = s.id) > 10
ON CONFLICT DO NOTHING;

-- 5) Create clinical cases
...
-- 6) Link cases to courses via case_assignments
...
-- 7) Add case_files (placeholder URLs)
...
INSERT INTO public.case_files (case_id, uploaded_by, file_name, file_url, file_type)
SELECT cc.id, '22222222-2222-2222-2222-222222222222', 'labs.pdf', 'https://example.com/sample-labs.pdf', 'otro'
FROM public.clinical_cases cc
WHERE cc.title = 'Shock Hipovolémico'
ON CONFLICT DO NOTHING;

-- 8) Chat messages and attachments
...
-- 11) Evaluations and scores (simulate grading some students)
...
-- 13) Notifications for course members
